...
from A_dettlssumo import SUMO
from junction import Junction
from A_udp import UDP_Server, UDP_Client
import A_queuedata
...
class Main:
    ...
    def init_tls_server(self):
        self.tlsudpser = UDP_Server(config.tlsport, A_queuedata.tls_queue)
    ...
    def process_tlsqueue_grouped(self):
        ...
        while True:
            if not A_queuedata.tls_queue.empty():
                print("        6.1、当前消息队列中信号灯色的数量：", A_queuedata.tls_queue.qsize())
                try:
                    tlsbin = A_queuedata.tls_queue.get()
                    ...
                    if str(tlsinfo[0]) in config.junctions:  # 仅处理关注的路口
                        ...
                        A_queuedata.ryg_queue.put((tlsid, rygstr))
    ...
    def process_flowQueue(self):
        while True:
            if not A_queuedata.flow_queue.empty():
                tmp = A_queuedata.flow_queue.get()
